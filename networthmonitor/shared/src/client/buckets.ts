import { deleteBucketApi, getBucketAccountsApi, getBucketsApi, patchBucketApi, postBucketApi, putBucketAccountsApi } from "../api";
...
export const getBucketAccounts = async (id: string): Promise<BucketAccountsResponse> => {
  ...
  getCache().set(
    CACHE_KEYS.buckets,
    existing.map((b) => (b.id === id ? { ...b, accountIds } : b)),
    getCacheExpiry(),
  );
...
export const postBucket = async (...args: Parameters<typeof postBucketApi.invoke>): Promise<Awaited<ReturnType<typeof postBucketApi.invoke>>> => {
...
export const patchBucket = async (...args: Parameters<typeof patchBucketApi.invoke>): Promise<Awaited<ReturnType<typeof patchBucketApi.invoke>>> => {
...
export const deleteBucket = async (...args: Parameters<typeof deleteBucketApi.invoke>): Promise<void> => {
  await deleteBucketApi.invoke(...args);
  removeBucketFromCache(args[0]!.id);
};

// PUT /buckets/:id/accounts - set accounts for bucket
export const putBucketAccounts = async (id: string, accountIds: string[]): Promise<void> => {
...
const updateBucketAccountIds = (id: string, accountIds: string[]) => {
  ...
  getCache().set(
    CACHE_KEYS.buckets,
    buckets.map((b) => (b.id === id ? { ...b, accountIds } : b)),
    getCacheExpiry(),
  );
};
