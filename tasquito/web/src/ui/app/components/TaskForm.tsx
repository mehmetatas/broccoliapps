...
export const TaskForm = ({ onSubmit, placeholder = "What needs to be done?" }: TaskFormProps) => {
  ...
  return (
    <form onSubmit={handleSubmit} class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700 p-5">
      ...
      {title.length >= LIMITS.MAX_TASK_TITLE_LENGTH && <p class="mt-1 text-xs text-neutral-400">Character limit reached</p>}
      ...
      {isExpanded && (
        <div class="mt-4 space-y-4">
          ...
          {/* Due Date */}
          <div>
            <h4 class="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wide mb-2">Due Date</h4>
            <DatePicker value={dueDate} onChange={setDueDate} />
          </div>

          {/* Subtasks Section */}
          <div>
            <h4 class="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wide mb-3">Subtasks</h4>

            {/* Existing subtasks */}
            {subtasks.length > 0 && (
              <div class="space-y-0 mb-0">
                {subtasks.map((subtask, index) => (
                  <div key={index} class="flex items-center justify-between py-1 px-1 group">
            ...
            {subtasks.length >= LIMITS.MAX_SUBTASKS_PER_TASK ? (
              ...
              <div>
                ...
                />
                {newSubtaskTitle.length >= LIMITS.MAX_SUBTASK_TITLE_LENGTH && <p class="mt-1 text-xs text-neutral-400">Character limit reached</p>}
              </div>
            )}
          </div>
